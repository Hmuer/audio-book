...
>
> **后续决策（2026-09-20 更新，见 `.trae/specs/optimization-plan/plan.md` 批次 3.5）**：
> - **播客永久不做**：全库从未落地过 Podcast 相关代码（无 provider/service/路由/配置），笔记 §7 的 API 速查仅作资料保留；此前唯一实装过的「节目化」能力是 Seed-Audio 多播剧（multicast），已随 P0-5 与 E-6 彻底清理。
> - **TTS 只保留豆包 2.0 + 声音复刻 2.0**：账号只开通这两项资源；1.0 小模型音色（`BV*`）已整表物理删除（94 条），且其不支持语音指令与标签。请求 1.0 资源实测返回 HTTP 403 + `code=45000030`。
> - **MiniMax 语音合成已弃用**（`MiniMaxLLMProvider` 保留，LLM 仍走它）。
> - **新增能力**：prepare 阶段用 LLM 为每句对白生成豆包 2.0 语音指令（`context_texts`），逐段落库并下发（见批次 3.5 工作流 ③）。语音标签 CoT 尚未启用。
>
> **优先级图例**：
> - 🔴 P0 = 必修（不修就跑不通）
> - 🟡 P1 = 推荐（P0 跑通后自然衔接）
